 ...
 */
internal class ParcelableListSerializer<T : Parcelable>(
    private val creator: Parcelable.Creator<T>
) : LocalSerializer<List<T>> {

    @Throws(IOException::class)
    override fun save(outputStream: OutputStream, obj: List<T>) {
        outputStream.write(ParcelableUtils.marshallList(obj))
        outputStream.flush()
    }
    ...
    override fun load(inputStream: InputStream): List<T>? {
        return inputStream.readBytes().let {
            ParcelableUtils.unmarshallList(it, creator)
        }
    }
}
