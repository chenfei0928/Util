...
import io.github.chenfei0928.util.kotlin.safeInvoke
...
object ListenersProxy {
    ...
    inline fun <reified T : Any> newListenersProxy(listenerImpls: MutableCollection<T> = arrayListOf()) =
    ...
    fun <T : Any> newEmptyListener(clazz: Class<T>): T {
    ...
    inline fun <reified T : Any> newImplByGetter(getter: Lazy<T>): T =
        newImplByGetter(T::class.java) { getter.value }

    inline fun <reified T : Any> newImplByGetter(crossinline getter: () -> T): T =
        newImplByGetter(T::class.java, getter)

    /**
     * 创建一个懒获取的代理实现，该代理对象可能不会实时获取到(通过一段代码来find或创建)或为懒加载
     */
    inline fun <T : Any> newImplByGetter(clazz: Class<T>, crossinline getter: () -> T): T {
        return Proxy.newProxyInstance(
